...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        low_mag_SpinBox = tk.Spinbox(labelframe_SlitConf,
                                      ...
                                      textvariable=self.check_valid_mags, width=5,
                                      from_=0, to=1080)
...
# #===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#===#=====
        ...
        GS_chkb = tk.Checkbutton(labelframe_SlitConf, text="Guide Star Pickup",
                               variable=self.GuideStarPickup_ChkBox_Enabled, command=self.pick_cb)
        GS_chkb.place(x=220, y=0)
...
#        self.string_DEC.set("-11.62305")#Sombrero
        ...
        button_RADEC_to_SOAR = tk.Button(labelframe_SlitConf, text="Accept Guide Star", bd=3,
                             command=self.send_RADEC_to_SOAR)
        button_RADEC_to_SOAR.place(x=250, y=98)
...
#        self.root.title(self.fullpath_FITSfilename)
    ...
    def SkyMapper_query_GuideStar(self):
        ...
        with fits.open(filepath.name) as hdu_in:
            ...
            fits.writeto(SkyMapperProduced, data,
...
#                     header=output_header, overwrite=True)
        ...
        table_full = pd.read_csv(tmp_file.name)
        
        self.table = table_full[ (table_full['i_psf']>10) & (table_full['i_psf']<13) ]
        ...
        for i in range(nstar):
            ...
            if  ( (x>round(data.shape[0]/4)) and (x<round(data.shape[0]*3/4)) and (y>round(data.shape[0]/4)) and (y<round(data.shape[0]*3/4)) ):
                i_drop.append(self.table.index[i])
                continue
            elif  ( (x<0) or (x>round(data.shape[0])) or (y<0) or (y>round(data.shape[0])) ):
                i_drop.append(self.table.index[i])
                continue
        ...
        box_region = RectanglePixelRegion(center=PixCoord(x=round(data.shape[0]/2), y=round(data.shape[1]/2)),
                          width=round(data.shape[0]/2), height=round(data.shape[1]/2),
                          angle=0*u.deg)  
