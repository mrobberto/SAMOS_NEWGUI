...
class HadamardPage(SAMOSFrame):
    def __init__(self, parent, container, **kwargs):
        super().__init__(parent, container, "Hadamard Sequences", **kwargs)
        self.map = None
        self.logger.info("Initializing Hadamdard Sequence Page")

        # Initialization Buttons
        frame = ttk.LabelFrame(self.main_frame, text="Initialize Components", borderwidth=3)
        frame.grid(row=0, column=0, sticky=TK_STICKY_ALL)
        w = ttk.Button(button_frame, text="Initialize DMD", command=self.init_dmd, bootstyle="success")
        w.grid(row=0, column=0, sticky=TK_STICKY_ALL)
        self.check_widgets[w] = [("condition", self.DMD, "is_on", False)]
        w = ttk.Button(button_frame, text="Initialize SAMI", command=self.init_sami, bootstyle="success")
        w.grid(row=1, column=0, sticky=TK_STICKY_ALL)
        self.check_widgets[w] = [("condition", self.SAMI, "connected", False)]
    ...
    def create_slits(self):
        self.map_filename.set("none")
        self.map_filename_path = None
        self.slits_filename_path = Path(asksaveasfilename(initialdir=self.PAR.locations['nightly_dir'], title="Create a new Slits file"))
