...
class PanStarrs():
    ...
    def getcolorim(self,ra, dec, size=240, output_size=None, filters="grizy", format="jpg"):
        ...
        url = self.geturl(ra,dec,size=size,filters=filters,output_size=output_size,format=format,color=True)
        r = requests.get(url)
        im = Image.open(BytesIO(r.content))
        return im
    
    
    def getgrayim(self,ra, dec, size=240, output_size=None, filter="g", format="jpg"):
        ...
        url = self.geturl(ra,dec,size=size,filters=filter,output_size=output_size,format=format)
