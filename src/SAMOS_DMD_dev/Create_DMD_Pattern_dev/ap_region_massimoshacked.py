...
from regions import PixCoord #added 2/6/23; needed to recognize dc.Rectangle
...
def ginga_canvas_object_to_astropy_region(obj, frame='icrs', logger=None):
    ...
    elif isinstance(obj, (dc.Rectangle,)):
        center = obj.get_center_pt()
        width = abs(obj.x2-obj.x1)
        height = abs(obj.y1-obj.y2)
        r = regions.RectanglePixelRegion(center=regions.PixCoord(center[0],center[1]),
                                         width = width,
                                         height = height)

    elif isinstance(obj, (dc.Slit,)):
        center = obj.get_center_pt()
        width = abs(obj.x2-obj.x1)
        height = abs(obj.y1-obj.y2)
        r = regions.RectanglePixelRegion(center=regions.PixCoord(center[0],center[1]),
