...
"""
Simbad_name = "NGC 346"
Target_name = "NGC 346" #"Simbad_Name"
...
"""
#CHECK: Extract generic table (e.g. 2MASS sources) fromm VizieR
"""
#table=""
...
conesearch_radius = 2 / 60.0  # in degrees
conesearch_center = radec_center
...
conesearch_records
table=conesearch_records.to_table()
pd_table = table.to_pandas()
pandacat = pd_table.rename(columns={'_RAJ2000': 'RA', '_DEJ2000': 'DEC'})
#set the weight as the distance from the line passing through (0,20), (1,18) in Fig. 12 of 
#Sabbi et al. https://iopscience.iop.org/article/10.1086/509257/pdf
x=pandacat['F555W']-pandacat['F814W']
y=pandacat['F555W']
A=2
B=1
C=-20
...
pandacat['d']=d
table=Table.from_pandas(pandacat)
...
#or
catfile  = './Westerlund1/Wd1_HSTandSOFIand2MASSandGAIA_MRCut.csv'
pandacat = pd.read_csv(catfile)
#check...
pandacat.head()
# If needed, you can rename columns
pandacat = pandacat.rename(columns={'ra_Gaia': 'RA', 'dec_Gaia': 'DEC'})
...
ax,wcs = Cat.get_2Mass_image(radec_center)
...
"""
WE work with Pandas table, and prepare for the weights/priority sorting
"""
...
arr=np.array(main_pdtable["Weight"])
grouped_array = divide_array(arr)
print(grouped_array)
main_pdtable["Weight"] = grouped_array
main_pdtable.to_csv(os.path.join(dir_name,"table_to_be_ranked_w.csv"))
print(main_pdtable)

"""
Now you may open the table_to_be_ranked.csv and edit to set different weights
...
#we MUST sort again the table, weight first, then RA as we disperse the specrra along DEC

main_pdtable_ranked  = main_pdtable_w.sort_values(by=['Weight','RA'],ascending=[True, False])
