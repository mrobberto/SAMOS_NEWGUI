...
for k in range(Nr_of_region_files): #k is the iterator labeling the region files
    ...
    for i in range(n_trials):
        ...
        xc = ((main_pdtable_ranked_internal['RA']-ra_center)*3600*6).astype(int)    #distance in pixel from the center field
        yc = ((main_pdtable_ranked_internal['DEC']-dec_center)*3600*6).astype(int) 
        length=2000
        spacing = 2 #pixel
        source_separation = (Slit_Length_pix + spacing) #e.g. 9 pixel slit length, + 2 spacing =  11 pixels separation center to center
...
# _|       fullarray = np.zeros((1100,4096))
        ...
        for j in range(Max_nr_of_targets):  #len(main_pdtable_ranked_internal):
            ...
            if main_pdtable_ranked_internal.at[j,'mask'] != -1:
                 ...
                 #print(main_pdtable_ranked_internal.iloc[j]['mask'])
                 continue                
            #            print(j,yc[j],int(length/2),xc[j],int(source_separation/2))
#_|            
            x0 = (yc[j]-int(source_separation/2)+550)
            x1 = (yc[j]+int(source_separation/2)+551)
            y0 = (xc[j]-int(length/2)+2048)
            y1 = (xc[j]+int(length/2)+2048)
